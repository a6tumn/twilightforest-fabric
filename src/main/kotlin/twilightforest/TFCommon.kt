...
object TFCommon : ModInitializer{
	const val ID: String = "twilightforest"

	private const val MODEL_DIR = "textures/entity/"
	...
	override fun onInitialize() {
		Reflection.initialize(ConfigSetup::class.java)
	...
	private fun setupCommonEvents() {
		LootEventListeners.init()
		CharmEventListeners.init()
		HostileMountEventListeners.init()
		MiscEventListeners.init()
		CapabilityEventListeners.init()
		ToolEventListeners.init()
		ProgressionEventListeners.init()
		TravellersGearEventListeners.init()
		EntityEventListeners.init()
	}
	...
	private fun registerBiomeSource() {
		Registry.register(
			BuiltInRegistries.BIOME_SOURCE,
			prefix("twilight_biomes"),
	...
	private fun registerGiantToolDropConversions() {
		LootEventListeners.GIANT_BLOCK_CONVERSIONS[Blocks.COBBLESTONE] = TFBlocks.GIANT_COBBLESTONE.asItem()
		LootEventListeners.GIANT_BLOCK_CONVERSIONS[Blocks.OAK_LOG] = TFBlocks.GIANT_LOG.asItem()
		LootEventListeners.GIANT_BLOCK_CONVERSIONS[Blocks.OAK_LEAVES] = TFBlocks.GIANT_LEAVES.asItem()
		LootEventListeners.GIANT_BLOCK_CONVERSIONS[Blocks.OBSIDIAN] = TFBlocks.GIANT_OBSIDIAN.asItem()
	}

	private fun registerJarLids() {
	...
	private fun registerFlammableBlocks() {
		val flammableBlockRegistry = FlammableBlockRegistry.getDefaultInstance()
	...
	private fun registerCompostables() {
	...
	private fun registerFuels() {
		FuelValueEvents.BUILD.register(FuelValueEvents.BuildCallback {
			builder: FuelValues.Builder, _: FuelValueEvents.Context? -> builder.add(TFItemTags.BANISTERS, 300)
		})
	}

	private fun registerParrotImitations() {
		Parrot.MOB_SOUND_MAP[TFEntities.ALPHA_YETI] = TFSounds.ALPHA_YETI_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.BLOCKCHAIN_GOBLIN] = TFSounds.REDCAP_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.CARMINITE_BROODLING] = SoundEvents.PARROT_IMITATE_SPIDER
		Parrot.MOB_SOUND_MAP[TFEntities.CARMINITE_GOLEM] = TFSounds.CARMINITE_GOLEM_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.FIRE_BEETLE] = SoundEvents.PARROT_IMITATE_SPIDER
		Parrot.MOB_SOUND_MAP[TFEntities.CARMINITE_GHASTLING] = SoundEvents.PARROT_IMITATE_GHAST
		Parrot.MOB_SOUND_MAP[TFEntities.CARMINITE_GHASTGUARD] = SoundEvents.PARROT_IMITATE_GHAST
		Parrot.MOB_SOUND_MAP[TFEntities.HEDGE_SPIDER] = SoundEvents.PARROT_IMITATE_SPIDER
		Parrot.MOB_SOUND_MAP[TFEntities.HELMET_CRAB] = SoundEvents.PARROT_IMITATE_SPIDER
		Parrot.MOB_SOUND_MAP[TFEntities.HOSTILE_WOLF] = TFSounds.HOSTILE_WOLF_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.HYDRA] = TFSounds.HYDRA_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.STABLE_ICE_CORE] = TFSounds.ICE_CORE_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.KING_SPIDER] = SoundEvents.PARROT_IMITATE_SPIDER
		Parrot.MOB_SOUND_MAP[TFEntities.KOBOLD] = TFSounds.KOBOLD_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.LICH] = SoundEvents.PARROT_IMITATE_BLAZE
		Parrot.MOB_SOUND_MAP[TFEntities.MAZE_SLIME] = SoundEvents.PARROT_IMITATE_SLIME
		Parrot.MOB_SOUND_MAP[TFEntities.LICH_MINION] = SoundEvents.PARROT_IMITATE_ZOMBIE
		Parrot.MOB_SOUND_MAP[TFEntities.MINOSHROOM] = TFSounds.MINOTAUR_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.MINOTAUR] = TFSounds.MINOTAUR_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.MIST_WOLF] = TFSounds.HOSTILE_WOLF_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.MOSQUITO_SWARM] = TFSounds.MOSQUITO_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.NAGA] = TFSounds.NAGA_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.KNIGHT_PHANTOM] = TFSounds.WRAITH_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.PINCH_BEETLE] = SoundEvents.PARROT_IMITATE_SPIDER
		Parrot.MOB_SOUND_MAP[TFEntities.REDCAP] = TFSounds.REDCAP_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.REDCAP_SAPPER] = TFSounds.REDCAP_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.SKELETON_DRUID] = SoundEvents.PARROT_IMITATE_SKELETON
		Parrot.MOB_SOUND_MAP[TFEntities.SLIME_BEETLE] = SoundEvents.PARROT_IMITATE_SLIME
		Parrot.MOB_SOUND_MAP[TFEntities.SNOW_GUARDIAN] = TFSounds.ICE_CORE_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.SNOW_QUEEN] = TFSounds.ICE_CORE_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.SWARM_SPIDER] = SoundEvents.PARROT_IMITATE_SPIDER
		Parrot.MOB_SOUND_MAP[TFEntities.TOWERWOOD_BORER] = SoundEvents.PARROT_IMITATE_SILVERFISH
		Parrot.MOB_SOUND_MAP[TFEntities.DEATH_TOME] = TFSounds.DEATH_TOME_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.UR_GHAST] = SoundEvents.PARROT_IMITATE_GHAST
		Parrot.MOB_SOUND_MAP[TFEntities.WINTER_WOLF] = TFSounds.HOSTILE_WOLF_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.WRAITH] = TFSounds.WRAITH_PARROT.value()
		Parrot.MOB_SOUND_MAP[TFEntities.YETI] = TFSounds.ALPHA_YETI_PARROT.value()
	}

	private fun registerDispenseBehaviors() {
		DispenserBlock.registerBehavior(TFItems.MOONWORM_QUEEN, object : DamageableStackDispenseBehavior() {
			override fun getProjectileEntity(level: Level, position: Position, stack: ItemStack): Projectile {
				return MoonwormShot(level, position.x(), position.y(), position.z())
			}

			override fun getDamageAmount(): Int {
				return 2
			}

			override fun getFiredSound(): SoundEvent {
				return TFSounds.MOONWORM_SQUISH.value()
			}
		})
		DispenserBlock.registerBehavior(TFItems.TWILIGHT_SCEPTER, object : DamageableStackDispenseBehavior() {
			override fun getProjectileEntity(level: Level, position: Position, stack: ItemStack): Projectile {
				return TwilightWandBolt(level, position.x(), position.y(), position.z())
			}
		...
		val iDispenseItemBehavior: DispenseItemBehavior = EquipmentDispenseItemBehavior.INSTANCE
		...
		DispenserBlock.registerBehavior(TFItems.PEACOCK_FEATHER_FAN.asItem(), FeatherFanDispenseBehavior())
		DispenserBlock.registerBehavior(TFItems.CRUMBLE_HORN.asItem(), CrumbleDispenseBehavior())
		DispenserBlock.registerBehavior(TFItems.TRANSFORMATION_POWDER.asItem(), TransformationDispenseBehavior())
		DispenserBlock.registerProjectileBehavior(TFItems.ICE_BOMB)
		DispenserBlock.registerBehavior(Items.CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.BLACK_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.GRAY_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.LIGHT_GRAY_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.WHITE_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.RED_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.ORANGE_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.YELLOW_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.GREEN_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.LIME_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.BLUE_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.CYAN_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.LIGHT_BLUE_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.PURPLE_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.MAGENTA_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.PINK_CANDLE, CandleDispenseBehavior())
		DispenserBlock.registerBehavior(Items.BROWN_CANDLE, CandleDispenseBehavior())
	}

	private fun registerCommands() {
		CommandRegistrationCallback.EVENT.register(CommandRegistrationCallback {
			dispatcher: CommandDispatcher<CommandSourceStack>, buildContext: CommandBuildContext, _: Commands.CommandSelection -> TFCommand.INSTANCE.register(dispatcher, buildContext)
		})
	}
	...
	private fun registerValidBlockEntityTypes() {
	...
	private fun registerReloadListeners() {
		ResourceLoader.get(PackType.SERVER_DATA).registerReloadListener(prefix("quest"), QuestReloadListener())
		ResourceLoader.get(PackType.SERVER_DATA).registerReloadListener(prefix("stalactite"), StalactiteReloadListener())
	...
	private fun registerCauldronInteractions() {
	...
	private fun registerItemStorage() {
		ItemStorage.SIDED.registerForBlockEntity(
			BiFunction {
				masonJar: MasonJarBlockEntity, side: Direction -> if (side == Direction.UP) masonJar.itemHandler else null
			},
			TFBlockEntities.MASON_JAR
		)
		ItemStorage.SIDED.registerForBlockEntity(
			BiFunction {
				entity: ChiseledCanopyShelfBlockEntity, side: Direction -> if (entity.blockState.getValue(ChiseledCanopyShelfBlock.SPAWNER)) null else ContainerStorage.of(entity, side)
			},
